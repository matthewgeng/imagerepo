...
from PIL import Image, ExifTags  # for image related processing, currently resizing images (pil keeps aspect ratio)
...
accepted_extensions_list = os.environ.get("ACCEPTED_EXTENSIONS") # use accepted extensions, this is saftey check after frontend check as well
if os.environ.get("FASTAPI_PROD") == "true": # production
    accepted_extensions_list = [] # TODO parse env list
...
async def preprocess_thumb_file(file, filename:str):
    ...
    image.save(imageBytes, format=image.format, exif=binary_exif)
...
async def upload_processed_thumb_file(file: UploadFile, name:str, ext:str, username: str):
...
async def upload(
    ...
    try: 
        for file in files:
            ...
            await upload_processed_file(file, name, ext, username)
            await upload_processed_thumb_file(file, name, ext, username)
        return {"success":True}
    except:
